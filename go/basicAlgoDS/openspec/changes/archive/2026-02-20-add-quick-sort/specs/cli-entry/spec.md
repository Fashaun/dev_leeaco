...
#### Scenario: Dynamic array list package name provided

- **WHEN** user runs `go run main.go dynamic_array_list`
- **THEN** the system SHALL call `dynamic_array_list.Run()` and display the dynamic array list operations demo output

#### Scenario: Quick sort package name provided

- **WHEN** user runs `go run main.go quick_sort`
- **THEN** the system SHALL call `quick_sort.Run()` and display the sorting demo output
